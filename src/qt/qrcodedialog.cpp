...
{
    QString ret = QString("kimdotcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non KDC unit input in lnReqAmount, we generate the URI with KDC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::KDC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
